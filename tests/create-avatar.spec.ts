...
test("custom images can be dropped or pasted from the clipboard", async ({
  ...
  // The automatic pipeline must produce a real transparent result for an
  // illustration, never leave the user with only the untouched background.
  await expect(
  ...
  const preview = page.getByRole("img", { name: "Avatar cutout preview" });
  await expect(preview).toBeVisible();
  const previewEdgeAlpha = await preview.evaluate((node) => {
  ...
  await expect(page.getByRole("button", { name: "CUTOUT", exact: true })).toHaveAttribute(
    "aria-pressed",
    "true"
  );
