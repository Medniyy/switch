...
test.describe("Banana Rain filter gating", () => {
  ...
  test("the birthday cake toggles from the same MonkeyDAO sheet and clears on switch", async ({ page }) => {
    await keepFullOn(page, { id: 77, collection: "smb-gen2", name: "SMB 77" });
    await page.getByRole("button", { name: "Filters" }).click();
    await page.getByRole("button", { name: /Blow Out The Candles/ }).click();
    expect(await page.evaluate(() => (window as unknown as { __appStore: { getState: () => { birthdayCake: boolean } } }).__appStore.getState().birthdayCake)).toBe(true);
    await page.evaluate(() => (window as unknown as { __appStore: { getState: () => { setSelectedNFT: (v: unknown) => void } } }).__appStore.getState().setSelectedNFT({ id: 1, collection: "test-plain", name: "x", image: "" }));
    expect(await page.evaluate(() => (window as unknown as { __appStore: { getState: () => { birthdayCake: boolean } } }).__appStore.getState().birthdayCake)).toBe(false);
  });
});

// ---------------------------------------------------------------------------
// Birthday cake game logic — driven headlessly through the dev seam, no
// canvas: update(dt, w, h, jawOpen) is the whole game.
// ---------------------------------------------------------------------------
...
async function driveCake(
  page: Page,
  steps: { ms: number; jaw: number }[]
): Promise<CakeProbe[]> {
  return page.evaluate((steps) => {
    const M = (window as unknown as {
      __switchMath: { CakeGame: new () => {
        update: (dt: number, w: number, h: number, jaw: number) => void;
        litCount: number;
        currentPhase: string;
      } };
    }).__switchMath;
    const game = new M.CakeGame();
    const out: { lit: number; phase: string }[] = [];
    for (const s of steps) {
      // Advance in 16ms frames like the render loop does.
      let left = s.ms;
      while (left > 0) {
        const dt = Math.min(16, left);
        game.update(dt, 390, 844, s.jaw);
        left -= dt;
      }
      out.push({ lit: game.litCount, phase: game.currentPhase });
    }
    return out;
  }, steps);
}

test("blows extinguish candles one at a time, talking does not", async ({ page }) => {
  await waitMath(page);
  const r = await driveCake(page, [
    { ms: 800, jaw: 0 }, // slide-in done
    { ms: 300, jaw: 0.3 }, // conversational mouth movement — no blow
    { ms: 100, jaw: 0.8 }, // one blow edge
    { ms: 100, jaw: 0.1 }, // release
    { ms: 100, jaw: 0.8 }, // second blow
  ]);
  expect(r[0]).toEqual({ lit: 6, phase: "play" });
  expect(r[1].lit).toBe(6); // talking never blows the cake out
  expect(r[2].lit).toBe(5);
  expect(r[4].lit).toBe(4);
});

test("a held blow sweeps the whole cake, then it celebrates and relights", async ({ page }) => {
  await waitMath(page);
  const r = await driveCake(page, [
    { ms: 800, jaw: 0 },
    { ms: 2600, jaw: 0.9 }, // hold the blow: 1 edge + repeats clears all 6
    { ms: 100, jaw: 0 },
    { ms: 4100, jaw: 0 }, // celebration runs out…
  ]);
  expect(r[1].lit).toBe(0);
  expect(r[1].phase).toBe("celebrate");
  // …and the loop restarts with every candle lit for the next take.
  expect(r[3]).toEqual({ lit: 6, phase: "play" });
});
